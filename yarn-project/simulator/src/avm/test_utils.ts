...
export function mockStorageRead(hs: HostStorage, value: Fr) {
  (hs as jest.Mocked<HostStorage>).publicStateDb.storageRead.mockResolvedValue(value);
}

export function mockStorageReadWithMap(hs: HostStorage, mockedStorage: Map<bigint, Fr>) {
  (hs as jest.Mocked<HostStorage>).publicStateDb.storageRead.mockImplementation((_address, slot) =>
...
export function mockNoteHashExists(hs: HostStorage, leafIndex: Fr, _value?: Fr) {
  (hs as jest.Mocked<HostStorage>).commitmentsDb.getCommitmentIndex.mockResolvedValue(leafIndex.toBigInt());
}

export function mockNullifierExists(hs: HostStorage, leafIndex: Fr, _value?: Fr) {
  (hs as jest.Mocked<HostStorage>).commitmentsDb.getNullifierIndex.mockResolvedValue(leafIndex.toBigInt());
}

export function mockL1ToL2MessageExists(hs: HostStorage, leafIndex: Fr, value: Fr, valueAtOtherIndices?: Fr) {
  (hs as jest.Mocked<HostStorage>).commitmentsDb.getL1ToL2LeafValue.mockImplementation((index: bigint) => {
...
export function mockGetContractInstance(hs: HostStorage, contractInstance: ContractInstanceWithAddress) {
  (hs as jest.Mocked<HostStorage>).contractsDb.getContractInstance.mockResolvedValue(contractInstance);
}
