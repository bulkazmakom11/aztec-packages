...
describe('Side Effect Trace', () => {
  ...
  const event = Fr.fromBuffer(randomBytes(Selector.SIZE));
  ...
  it('Should trace storage reads', () => {
    ...
    expect(pxResult.contractStorageReads).toEqual([
      ...
      expect(pxResult.avmCircuitHints.storageValues.items[0]).toEqual({ key: startCounterFr, value: value });
  });
  ...
  it('Should trace note hash checks', () => {
    ...
    expect(pxResult.avmCircuitHints.noteHashExists.items[0]).toEqual({ key: startCounterFr, value: new Fr(exists) });
  });
  ...
  it('Should trace nullifier checks', () => {
    ...
    expect(pxResult.avmCircuitHints.nullifierExists.items[0]).toEqual({ key: startCounterFr, value: new Fr(exists) });
  });
  ...
  it('Should trace L1ToL2 Message checks', () => {
    ...
    expect(pxResult.avmCircuitHints.l1ToL2MessageExists.items[0]).toEqual({
      key: startCounterFr,
      value: new Fr(exists),
    });
  });
  ...
  it('Should trace get contract instance', () => {
    ...
    expect(pxResult.avmCircuitHints.contractInstances.items[0]).toEqual({
